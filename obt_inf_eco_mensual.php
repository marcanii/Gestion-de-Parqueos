...
if (isset($_POST['mes'])) {
    ...
    $num_autos = "SELECT COUNT(*) AS total FROM parqueo WHERE MONTH(fecha) = '$mes'; ";
    $resultado = $con->query($num_autos);
    $row = $resultado->fetch_assoc();
    $total_autos = $row['total'];
    // sacar el total de dinero que se recaudo en el dia
    $total_dinero = "SELECT SUM(costo_total) AS total FROM parqueo WHERE MONTH(fecha) = '$mes'; ";
    ...
    if ($total_autos == 0 and $total_dinero == 0) {
        ...
        echo '<table class="table table-dark table-bordered" id="tableToExport">
                    ...
                    <tbody>
                        <tr>
                            <td>Numero de autos que ingresaron en el mes</td>
                            <td>' . $total_autos . '</td>
                        </tr>
                        <tr>
